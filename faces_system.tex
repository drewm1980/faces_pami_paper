...
occlusion or corruption by solving a simple convex program.  For this
...
image.  We argue that if one has the luxury of designinig the recognition algorithm,
but also the acquisition system, and the application demands a high recognition rate,
it is inadvisable to limit the gallery to a
...
mutually confounding issues of registration and illumination
...
{\bf DISCUSSION OF ZIHAN'S LBP IMPLEMENTATION GOES HERE}
\begin{table}
...
%\vspace{-.8in}
\end{figure}
Our algorithm is not designed for recognition when there is only a single
...
three standard subsets of the FERET \cite{phillips1998feret} dataset,
summarized in Table \ref{tab:FERET-recognition}.  
\begin{table}
\caption{Recognition rates on single gallery image FERET dataset}
\centerline{
\begin{tabular}{|c|c|c|c|c| }
\hline
