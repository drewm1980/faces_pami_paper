...
\end{itemize}
\begin{figure}
\centerline{\hspace{-0.1in}\includegraphics[height=2.3in]{figures_cvpr/camera_rig.pdf}\vspace{0mm}}
...
\centering
\begin{tabular}{cc}
\includegraphics[height=1in]{figures_cvpr/illum_results/coverage_sunset.pdf} &
\includegraphics[height=1in]{figures_cvpr/illum_results/granularity_sunset.pdf} \\
...
\hline
\end{tabular}
\label{tab:MultiPIE-recognition} } \vspace{-.4in}
\end{table}
...
dataset} \vspace{0mm} \centerline{
\begin{tabular}{|c|c|c|c|c| }
\hline
Recognition rate & fb & fc & dup1 & dup2 \\
\hline
\hline
LBP($d/m/i$) & 54.8 / {\bf 96.6} / 94.5 \% & 10.3 / {\bf 58.8} / 42.8\% & 29.8 / {\bf 71.6} / 46.5\% & 19.8 / {\bf 61.5} / 21.1\% \\
\hline
{Algorithm 1} & 95.2\% & 28.4\% & 46.1\% & 20.3\% \\
\hline
\end{tabular}
\label{tab:FERET-recognition} } \vspace{0mm}
\end{table}
...
\centerline{
	...
	{\small All algorithm with iterative alignment} & {\small Classical algorithms with manual alignment}
\\
	\end{tabular}
}
\caption{{\bf ROC curves} for subject validation on Multi-PIE database, (a) with iterative alignment,
and (b) for the classical algorithms with manual alignment (indicated by a subscript ``m'').}\label{fig:roc-multipie}
\vspace{-.4in}
\end{figure}
...
face images are the same subject. Figure \ref{fig:roc-multipie} should not be
confused with typical ROC curves for face similarity, e.g.,
\cite{PhillipsP2007}.} On the left we can see that the SCI
based recognition approach significantly outperforms the other
algorithms, including LBP, even when all algorithms are coupled
with our proposed iterative alignment.  In the right plot we
again see that classical algorithms, and even LBP, are very
sensitive to alignment.  Similar contrasts between our
algorithm and baseline algorithms were also observed for SRC in
...
\begin{figure}
\centerline{
\includegraphics[width=4in]{figures_pami/occ_block.png}\vspace{0mm}}
