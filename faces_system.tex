...
CMU Multi-PIE provides the most extensive test set among public datasets. This
database contains images of 337 subjects across simultaneous variation in pose,
expression, and illumination.  Of these 337 subjects, we use all of the 249
subjects present in Session 1 as the training set. The remaining 88 subjects
are treated as ``impostors'', or invalid images. For each of the 249 training
subjects, we include frontal images of 7 frontal illuminations,\footnote{They
are illuminations $\{0,1,7,13,14,16,18\}$ of \cite{Gross2008-FGR}. For each
directional illumination, we subtract the ambient-illuminated image 0.} taken
with neutral expression. As suggested by the work of
\cite{Georghiades2001-PAMI}, these extreme frontal illuminations would be
sufficient to linearly represent other frontal illuminations, as will also be
corroborated by the next experiment on our own dataset. For the test set, we
use all 20 illuminations from Sessions 2-4, which were recorded over a period
of several months. The dataset is challenging due to the large number of
subjects, and due to natural variation in subject appearance over time.  Line 5
of Table \ref{tab:MultiPIE-recognition} shows the result of our recognition
pipeline on each of the 3 testing sessions. Our algorithm achieves recognition
rates above $90\%$ for all three sessions.  For the test images, our iterative
alignment was initialized automatically via the Viola and Jones' face detector.

\begin{table}
\caption{Recognition rates on the Multi-PIE database for different pairings of alignment
...
well if the test image is not well aligned with the training. In
Table~\ref{tab:MultiPIE-recognition}, we report the results of
these classical algorithms with three types of testing image alignment: 
1.\ alignment from the Viola and Jones' detector,
2.\ alignment via manually selected outer eye corners, and 
3.\ the output of our iterative alignment algorithm.
...
greatly from being paired with our iterative alignment algorithm.
...
this paper, we follow the same steps as in \cite{Ahonen2006-PAMI} to construct
an LBP descriptor for each training and testing sample. The $80\times 60$ face
region is first divided into a regular $10\times 10$ grid of cells, each of
size $8\times 6$ pixels. Within each cell, the histogram of 59 uniform binary
patterns is then computed, where the patterns are generated by thresholding 8
neighboring pixels in a circle of radius 2 using the central pixel value.
Finally, the local histograms are concatenated to produce the global descriptor
vector. As suggested in \cite{Ahonen2006-PAMI}, the recognition is performed
using a nearest neighbor classifier with Chi square distance as the distance
measure and we report the recognition rates with the same two types of input as
before.  Recognition results for LBP on Multi-PIE are shown in
Table~\ref{tab:MultiPIE-recognition}.  We attribute the improvement in
performance of LBP over SRC in this experiment to its robustness to
illumination components that are not in the training set.  Some of the test
images used in this experiment are taken under illuminations not present in the
training subset.  Note that, like the other classical algorithms, the
performance of LBP is still sensitive to misalignment. This is notable given
that LBP is often applied without any special alignment.

\begin{table}
\caption{Recognition rates on the Multi-PIE database for \cite{Yang2010-CVPR} and variants
of Algorithm 1}
\vspace{-.2in}
\centerline{
...
To demonstrate that the sparse representation based recognition step is indeed
beneficial even when there are no impostors, we include results for recognition
based only on the alignment error residuals(i.e. $S=1$), shown in 
Table \ref{tab:MultiPIE-recognition2}, row 6.

We also compare our result to the recent work
...
images are the same subject. Figure 8 should not be confused with typical ROC
curves for face similarity, e.g., \cite{PhillipsP2007}.} On the left we can see
that the SCI based recognition approach significantly outperforms the other
algorithms, including LBP, even when all algorithms are coupled with our
proposed iterative alignment.  In the right plot we again see that classical
algorithms, and even LBP, are very sensitive to alignment.  Similar contrasts
between our algorithm and baseline algorithms were also observed for SRC in
\cite{Wright2009-PAMI}, though on much smaller datasets.
\begin{figure}
\centerline{
	\begin{tabular}{cc}
	\includegraphics[width=3in]{figures_pami/pami_roc_revision2.png} &
	\includegraphics[width=3in]{figures_pami/pami_roc2.png} \\
	{\small With iterative alignment} & {\small Classical Algorithms with manual alignment} 
\\
	\end{tabular}
}
\caption{{\bf ROC curves} for subject validation on Multi-PIE database, (a) with iterative alignment,
and (b) for the classical algorithms with manual alignment}\label{fig:roc-multipie}
%\vspace{-.6in}
\end{figure}
