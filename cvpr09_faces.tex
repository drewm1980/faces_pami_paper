...
\centering
\begin{tabular}{cc}
\includegraphics[height=0.9in]{Figures/promo/case1/detector.png}& \hspace{3mm}
\includegraphics[height=0.9in,width=1.7in]{Figures/promo/case1/sci_with_axis_face_case1.png} \\
\includegraphics[height=0.9in]{Figures/promo/alignment_and_detector.png}& \hspace{3mm}
\includegraphics[height=0.9in,width=1.7in]{Figures/promo/case2/sci_with_axis_face_case2.png} \\
\includegraphics[height=0.9in]{Figures/promo/case3/alignment.png} & \hspace{3mm}
\includegraphics[height=0.9in,width=1.7in]{Figures/promo/case3/sci_with_axis_face_case3.png}
\end{tabular}
...
\centering
\begin{tabular}{cccc}
\includegraphics[scale=0.214,clip=true]{figures/L1_cropped} &
\includegraphics[scale=0.75,clip=true]{figures/y_warp_L1} &
\includegraphics[scale=0.75,clip=true]{figures/y_hat_L1} &
\includegraphics[scale=0.75,clip=true]{figures/e_L1} \\
\includegraphics[scale=0.214,clip=true]{figures/L2_cropped} &
\includegraphics[scale=0.75,clip=true]{figures/y_warp_L2} &
\includegraphics[scale=0.75,clip=true]{figures/y_hat_L2} &
\includegraphics[scale=0.75,clip=true]{figures/e_L2} \\
(a) & (b) & (c) & (d)
\end{tabular}
...
\includegraphics[height=1.36in]{figures/translation_fig3.png}
%\centerline{\small a) $x$ \& $y$ translation}
\end{minipage}\hspace{3mm}
\begin{minipage}{1.5in}
\includegraphics[height=1.42in, width = 1.9in]{figures/translation_rotation_fig1.png}
%\centerline{\small b) $y$-translation \& rotation}
\end{minipage}
\vspace{.1in}
...
\begin{figure}[t]
\begin{tabular}{ccccc}
\includegraphics[scale=0.085,clip=true]{figures/5} &
\includegraphics[scale=0.085,clip=true]{figures/7} &
\includegraphics[scale=0.085,clip=true]{figures/09} &
\includegraphics[scale=0.085,clip=true]{figures/11} &
\includegraphics[scale=0.085,clip=true]{figures/13} \\
(a) & (b) & (c) & (d) & (e)\\
\includegraphics[scale=0.085,clip=true]{figures/15} &
\includegraphics[scale=0.085,clip=true]{figures/17} &
\includegraphics[scale=0.085,clip=true]{figures/19} &
\includegraphics[scale=0.085,clip=true]{figures/21} &
\includegraphics[scale=0.085,clip=true]{figures/3} \\
(f) & (g) & (h) & (i) & (j)
\end{tabular}
...
%Additional experimental results in the next section will further confirm this point. 
...
\end{itemize}
\begin{figure}
\centerline{\hspace{-0.1in}\includegraphics[width=2.3in]{figures/camera_rig.pdf}}
...
\includegraphics[height=1.1in]{figures/coverage_experiment_asplode.png}
\centerline{\small (a) Coverage Experiment}
\end{minipage}\hspace{5mm}
\begin{minipage}{1.5in}
\includegraphics[height=1.1in]{figures/final_cvpr_illuminations_asplode.png}
...
\begin{figure}[h]
\begin{minipage}{1.5in}
\includegraphics[height=1.3in]{figures/illum_results/coverage_sunset.pdf}
\centerline{\small (a) Coverage }
\end{minipage}\hspace{3mm}
\begin{minipage}{1.5in}
\includegraphics[height=1.3in]{figures/illum_results/granularity_sunset.pdf}
...
\begin{figure}[h]
\includegraphics[width=3.3in]{figures/training.png}
\end{figure}
\vspace{-6mm}
...
%Once we have sufficient number of illuminations, we may expect that any new illumination falls into the convex hull spanned by them, known as the illumination cone. In this case, when we try to find the sparse representation of a test image from equation \eqref{eqn:robust-l1}, we also expect the coefficients $\x$ to be non-negative. This ensures the representation to be physically meaningful -- an image that has negative values. This point is reflected in the linear program used in Algorithm 1. 

%In addition, the linear illumination model requires that the camera has captured the illuminance faithfully. But that is not the case for most cameras: older analog video cameras often use NTSC defined gamma compression, while most modern digital consumer and SLR cameras record in sRGB defined gamma compression.\footnote{For more info, see the website \url{http://en.wikipedia.org/wiki/Gamma_correction}}  Since not all algorithms depend on a linear model for the illumination, there are public face recognition databases that do not specify the gamma compression of their images.  The performance of the algorithm presented in this paper benefits greatly if proper gamma decompression has been applied to the training and testing images.


%{\bf Yi: show how sparse representation improves for subject X with the new training illuminations (Andrew).}

%\jw{I have not touched this section much.}
...
%A lot of algorithms make much stronger assumptions than linearity in the interest of simplifying the problem.  For example, if the object under study is convex, the lighting is distant, and the surface reflectance is Lambertian, it has been shown that most of the variation of the reflected light with respect to the surface normal will be concentrated in the low frequencies with respect to the direction of the surface normal.  This provides a good justification for relying on a reduced number of illuminations for your training set, provided that they are well chosen.  Indeed, when most of the illumination in the training and testing images is frontal (as it is in most public face recognition databases), the importance of non-convexity is significantly decreased.

%\jw{The below thoughts on Gamma compression should go somewhere in this section.}
%\paragraph{Compensating for Gamma Compression.}
%Any algorithm based on representing a testing image as a weighted sum of training images must make sure that the image intensity has been coded linearly with image intensity.  Unfortunately, this cannot be taken for granted.  While most modern machine vision cameras capture linearly in intensity, older analog video cameras often used NTSC defined gamma compression, while most modern digital consumer and SLR cameras record in sRGB defined gamma compression.  Since not all algorithms depend on a linear image coding, there are public face recognition databases that do not specify the gamma compression of their images.  The performance of the algorithm presented in this paper benefits greatly from proper gamma decompression of the training and testing images.

%
%%Improved robustness to illumination variation can be achieved by integrating the design of the algorithm with the design of the training image capture system.

%\subsection{Should Positivity in the Representation Coefficients be Enforced?}
%For non-convex objects, enforcing non-negativity of the representation is not sufficient to guarantee that the resulting image of the object is physical.  For example, consider a Lambertian planar object with a cylindrical hole drilled in it that is aligned with the optical axis of an orthographic camera.  Consider two images: one under illumination from a distant point source aligned with the hole, and one off of the hole axis.  Positively weighting the on-axis image and negatively weighting the on-axis image can result in an image where the bottom of the hole is more strongly illuminated than the surrounding plane.  The image is positive, but it is clearly non-physical.  It is worth emphasizing that this phenomenon is a result of some points on an object seeing a restricted subset of the illumination sphere,  and not a result of the directionality of the light chosen in the thought experiment.  

%		Why doesn't Spherical Harmonics solve the problem of what your training images should be?
%			-Illuminations must be non-negative.  Could take positive and negative parts of the image separately, but we're not allowing negative coefficients.

%	\subsection{Motivation}
%Unfortunately, {\em human faces are non-convex and non-lambertian enough to cause big problems for face recognition systems!}  When algorithm \ref{eqn:final_algorithm} is run on images of people taken under lighting conditions, the error image spends a lot of its energy on areas in the image where there is either a specularity (a violation of the Lambertian assumption), or self-shadowing (a violation of the assumption that the object is convex).
%	\subsection{Experiments}

%		\begin{itemize}
%		\item Show that a multii-pie-like subset of illuminations fails to represent lobby and outdoor images; show a single example of per-user alignment borking.
%		\item Show that a multii-pie-like subset of illuminations fails to represent lobby and outdoor images, compute (low) success rate of per-user alignment over a bunch of test images.
%		\item Show that the complete set of training illuminations succeeds on the same images; show a single example of per-user alignment working.
%		\item Show that the complete set of training illuminations succeeds on the same images, compute (high) success rate of per-user alignment over a bunch of test images. \end{itemize}

%		\subsection{Experimentally Nail down what training images are actually necessary}
%			\begin{itemize}
%				\item Increase coverage, expanding outwards from frontal.
%				\item Gradually increase granularity, show where performance jumps.  Have to express granularity in terms of percentage of the illumination sphere.
%				\item Test if partitions of the illumination sphere perform better than point sources at the centers of the regions.  Regions may be better due to lowpass effect on cast shadow boundaries.		
%			\end{itemize}
...
\paragraph{Subject validation.} We test the algorithms' ability to reject invalid images of the 88 subjects not appearing in the training database. Figure 8 (bottom) plots the receiver operating characteristic (ROC) curve for each algorithm.\footnote{Rejecting invalid images not in the entire database is much more difficult than deciding if two face images are the same subject. Figure 8 should not be confused with typical ROC curves for face similarity, e.g., \cite{PhillipsP2007}.} Similar contrasts between our algorithm and baseline algorithms were also observed for SRC in \cite{Wright2008-PAMI}, though on much smaller datasets. 
...
\centerline{
\includegraphics[width=2.8in]{figures/roc3.png} 
}
...
\paragraph{Cause of errors.} Our algorithm's errors are mostly caused by a few subjects who significantly change their appearances between sessions (such as hair, facial hair, and eyeglasses). Some representative examples are shown in Figure \ref{fig:failed-examples}. In fact, for those subjects, alignment and recognition fail on almost all test illuminations.\vspace{-3mm} %This may be due the limited number of illuminations present in the training. \vspace{-4mm}
\begin{figure}[t]
\centering
\includegraphics[scale=0.2320,clip=true]{figures/079_01_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/130_01_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/163_01_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/175_01_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/118_01_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/223_01_01_051_08.png} \\
\includegraphics[scale=0.2320,clip=true]{figures/079_02_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/130_02_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/163_02_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/175_02_01_051_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/118_02_01_140_08.png} 
\includegraphics[scale=0.2320,clip=true]{figures/223_02_01_140_08.png} 
...
Now we use all 74 subjects and 38 illuminations in the training and test on 593 images taken under a variety of conditions. Based on the main variability in the test images, we have partitioned them into five main categories:%\vspace{-2mm}
\begin{small}
\begin{description}
\item[C1:] 242 images of 47 subjects without eyeglasses, generally frontal view, under a variety of practical illuminations (indoor and outdoor) (Fig. 10, row 1). %\vspace{-2mm}
\item[C2:] 109 images of 23 subjects with eyeglasses (Fig. 10, row 2).%\vspace{-2mm}
\item[C3:] 19 images of 14 subjects with sunglasses (Fig. 10, row 3).%\vspace{-2mm}
\item[C4:] 100 images of 40 subjects with noticeable expressions, poses, mild blur, and sometimes occlusion (Fig. 11, both rows).%\vspace{-2mm}
\item[C5:] 123 images of 17 subjects with little control (out of focus, motion blur, significant pose, large occlusion, funny faces, extreme expressions) (Fig. 12, both rows).%\vspace{-2mm}
\end{description}
\end{small}
...
\begin{figure}
\centering
\includegraphics[scale=0.25,clip=true]{figures/examples/1/DSC_1319.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/1/DSC_1531.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/1/DSC_1574.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/1/DSC_1622.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/1/DSC_1786.jpg} \\
\includegraphics[scale=0.25,clip=true]{figures/examples/2/DSC_1448.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/2/DSC_1585.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/2/DSC_1588.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/2/DSC_1666.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/2/DSC_1874.jpg} \\
\includegraphics[scale=0.25,clip=true]{figures/examples/3/DSC_1572.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/3/DSC_1587.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/3/DSC_1623.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/3/DSC_1652.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/3/DSC_1675.jpg} 
 \caption{{\bf Representative examples of categories 1-3}. One row for each category.\vspace{1.0mm}}\label{fig:examples1-3}
%\end{figure}
%\begin{figure}[h]
\centering
\includegraphics[scale=0.25,clip=true]{figures/examples/4/success/DSC_1579.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/success/DSC_1839.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/success/DSC_1883.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/success/DSC_1922.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/success/DSC_2000.jpg} \\
\includegraphics[scale=0.25,clip=true]{figures/examples/4/failure/DSC_1568.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/failure/DSC_1578.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/failure/DSC_1500.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/failure/DSC_1930.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/4/failure/DSC_1985.jpg} 
 \caption{{\bf Representative examples of category 4}. Top row: successful examples. Bottom row: failures.}\label{fig:examples4}
\vspace{1mm}
\centering
\includegraphics[scale=0.25,clip=true]{figures/examples/5/success/DSC_1664.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/success/DSC_1777.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/success/DSC_1871.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/success/DSC_1913.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/success/DSC_2005.jpg} \\
\includegraphics[scale=0.25,clip=true]{figures/examples/5/failure/DSC_1669.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/failure/DSC_1800.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/failure/DSC_1870.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/failure/DSC_1917.jpg} 
\includegraphics[scale=0.25,clip=true]{figures/examples/5/failure/DSC_2008.jpg} 
