...
\DIFdelbegin %DIFDELCMD < \PARstart{T}{here} %%%
\DIFdel{is a historical tendency for face recognition algorithms to work well under laboratory conditions but degrade when tested in less-controlled environments.While there have been several high profile failed trials }\DIFdelend \DIFaddbegin \PARstart{F}{ace} \DIFadd{recognition applications to date have fallen into roughly two
...
organization (e.g.}\ \DIFadd{Google Picassa, Microsoft Photo Gallery, and Apple iPhoto).
...
field }\DIFaddend of face recognition \DIFdelbegin \DIFdel{technology for extremely difficult mass surveillance / watch-list applications, face recognition has not even seen widespread applications in more tractable settings, such as }\DIFdelend \DIFaddbegin \DIFadd{research.  However, there are many face recognition
...
system rather than adversaries.  These applications include }\DIFaddend access control for
\DIFdelbegin \DIFdel{buildings}\DIFdelend \DIFaddbegin \DIFadd{secure facilities (e.g., prisons, office buildings)}\DIFaddend , computer systems,
automobiles, or \DIFdelbegin \DIFdel{even }\DIFdelend automatic teller machines\DIFdelbegin \DIFdel{.}\footnote{\DIFdel{Face recognition scenarios where both gallery and test data are uncontrolled is an active research area as well }%DIFDELCMD < \cite{LFW}%%%
\DIFdel{.}}  %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{, where controlled gallery images can
be obtained in advance.  }\DIFaddend These applications are very interesting due to their
potential sociological impact.  \DIFdelbegin \DIFdel{They are also qualitatively different from surveillance applications because they demand extremely reliable systems, and because }\DIFdelend \DIFaddbegin \DIFadd{Since }\DIFaddend the gallery subjects are allies, rather
than opponents, of the recognition system\DIFdelbegin \DIFdel{.  This }\DIFdelend \DIFaddbegin \DIFadd{, this }\DIFaddend creates the possibility of
carefully controlling the acquisition of the training data\DIFdelbegin \DIFdel{, even if the testing data may be less
controlledfor usability reasons}\DIFdelend \DIFaddbegin \DIFadd{. While the same can
...
but still important, scenario}\DIFaddend . 

\DIFdelbegin \DIFdel{Classical holistic
subspace based }\DIFdelend \DIFaddbegin \DIFadd{Very few recognition systems specifically target applications where many well-
controlled training images are available.  Of these, the classical holistic
subspace-based face recognition }\DIFaddend methods \cite{Turk1991-CVPR,Belhumeur1997-PAMI}
...
shown to be extremely brittle not only to alignment variation, but to \DIFdelbegin \DIFdel{occlusions}\DIFdelend \DIFaddbegin \DIFadd{even
minor occlusions caused by, say, a wisp of hair, a blinked eye, or mouth that
is slightly open}\DIFaddend . One promising recent direction, set forth in
...
%DIF > For this reason, the final recognition stage of our proposed system will
%DIF > consist of SRC, and the alignment stage that precedes it will be based on
%DIF > similar techniques.
...
\end{tabular} \caption{{\bf \DIFdelbeginFL \DIFdelFL{Compound effect }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Effects }\DIFaddendFL of registration and illumination \DIFaddbeginFL \DIFaddFL{on
Recognition}\DIFaddendFL }. \DIFdelbeginFL \DIFdelFL{The task is to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{In this example we }\DIFaddendFL identify the girl among 20 subjects, by
...
Section \ref{sec:illumination}). {\bf Bottom:} \DIFdelbeginFL \DIFdelFL{Informative representation obtained by using both well-aligned }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL input face \DIFaddbeginFL \DIFaddFL{is well aligned }\DIFaddendFL and 
\DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL sufficient \DIFaddbeginFL \DIFaddFL{set }\DIFaddendFL (all 38) \DIFaddbeginFL \DIFaddFL{of
}\DIFaddendFL illuminations \DIFaddbeginFL \DIFaddFL{are used }\DIFaddendFL in the training. \DIFaddbeginFL \DIFaddFL{Both are necessary for correct recognition
using SRC.}\DIFaddendFL } \label{fig:promo}
%DIF < \vspace{-.5in}
...
%DIFDELCMD < %%%
\DIFdelend %\subsection{Compound Effect of Registration and Illumination}
\DIFaddbegin 

\DIFaddend Unfortunately, while \DIFdelbegin \DIFdel{SRC }\DIFdelend \DIFaddbegin \DIFadd{the work }\cite{Wright2009-PAMI} \DIFaddend achieves impressive results, it does not deal with misalignment
between the test and training images\DIFdelbegin \DIFdel{.  Furthermore, the experiments were carried out on public
datasets
taken under controlled laboratory conditions
such as
Extended Yale B }%DIFDELCMD < \cite{Georghiades2001-PAMI}%%%
\DIFdel{, which do not contain realistic test images .
}\DIFdelend \DIFaddbegin \DIFadd{, and it requires a rich set of illuminations
in the gallery images for good performance.  
%DIF > Furthermore, the experiments were carried out on public datasets taken under
%DIF > controlled laboratory conditions such as Extended Yale B
%DIF > \cite{Georghiades2001-PAMI}, which do not contain realistic test images.
}\DIFaddend %two critical aspects of real world face recognition must be addressed:  significant variations in both the {\em image domain} and in
%the {\em image value}.
We illustrate \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the compounded effect of both alignment and illumination 
}\DIFaddend with an example in Figure \ref{fig:promo}.
...
the \DIFdelbegin \DIFdel{representation }\DIFdelend \DIFaddbegin \DIFadd{sparse representation obtained using the method of }\cite{Wright2009-PAMI} \DIFaddend is no longer informative, 
even if sufficient illuminations are present in the training, as shown in Figure
\ref{fig:promo}(top). \DIFdelbegin \DIFdel{In addition}\DIFdelend \DIFaddbegin \DIFadd{Additionally}\DIFaddend , in order to \DIFdelbegin \DIFdel{linearly }\DIFdelend span
the illuminations of a typical indoor (or outdoor) environment,
illuminations from behind the subject are \DIFdelbegin \DIFdel{also }\DIFdelend needed in the
training \DIFaddbegin \DIFadd{set}\DIFaddend . Otherwise, even for perfectly aligned test images,
the \DIFdelbegin \DIFdel{representation }\DIFdelend \DIFaddbegin \DIFadd{sparse representation obtained using }\cite{Wright2009-PAMI} \DIFaddend will not necessarily be sparse or
informative, as shown by the example in Figure
\ref{fig:promo}(middle). Clearly, both good alignment, as well
as sufficient training images are needed \DIFaddbegin \DIFadd{to ensure success of the sparsity-based 
recognition method proposed by }\cite{Wright2009-PAMI}\DIFaddend .  
...
\DIFaddbegin \DIFadd{In this paper, we examine how to handle alignment and illumination simultaneously
in the sparse representation framework, bringing the method proposed in }\cite{Wright2009-PAMI}
\DIFadd{closer to practical use.
}\DIFaddend 
...
techniques.  This fact immediately distinguishes our \DIFdelbegin \DIFdel{technique }\DIFdelend \DIFaddbegin \DIFadd{approach }\DIFaddend from systems
that either require a 3D data as an input, or attempt to estimate a 3D model
from \DIFdelbegin \DIFdel{input }\DIFdelend 2D \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{input }\DIFaddend \cite{amberg2007reconstructing,Blanz2003-PAMI}.  While these
techniques \DIFdelbegin \DIFdel{have been shown to }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend achieve better robustness to pose variation given
a sufficiently accurate 3D model, for access control applications where \DIFdelbegin \DIFdel{the user is cooperative in aligning their head to the camera, robustness to moderate misalignment is sufficient.  Furthermore, many 3D techniques rely on multiple views to estimate their model, and may have trouble if all of the training images are captured from one pose}\DIFdelend \DIFaddbegin \DIFadd{only 
moderate pose variation is present, the proposed method will be more
than sufficient}\DIFaddend .   Note that \DIFdelbegin \DIFdel{images of people's }\DIFdelend \DIFaddbegin \DIFadd{2D images of }\DIFaddend faces
under varying \DIFdelbegin \DIFdel{illumination contain }\DIFdelend \DIFaddbegin \DIFadd{illuminations already contain 3D }\DIFaddend shape-related \DIFdelbegin \DIFdel{discriminative information,}\DIFdelend \DIFaddbegin \DIFadd{information,}\footnote{\DIFadd{In principle,
one can recover the 3D shape of the face from multiple illuminations using photometric stereo.}} 
\DIFaddend and this information can be leveraged by 2D algorithms \DIFaddbegin \DIFadd{for alignment and recognition 
}\DIFaddend even if shape is not reconstructed explicitly. 
%DIF > In addition, the proposed sparsity-based robust
%DIF > alignment and recognition techniques are not limited to 2D data and can be easily extended
%DIF > to 3D data if necessary. 
...
unnecessary complexity to \DIFdelbegin \DIFdel{a system where subjects are willing to }\DIFdelend \DIFaddbegin \DIFadd{applications where subjects normally }\DIFaddend present a neutral \DIFaddbegin \DIFadd{face
or only have moderate }\DIFaddend expression. We prefer to focus on \DIFdelbegin \DIFdel{warpings }\DIFdelend \DIFaddbegin \DIFadd{deformations }\DIFaddend with far fewer 
\DIFdelbegin \DIFdel{parameters}\DIFdelend \DIFaddbegin \DIFadd{degrees of freedom, i.e. similarity transformations}\DIFaddend , and to use the training images themselves 
...
\DIFdelbegin \DIFdel{However, whereas }\DIFdelend \DIFaddbegin \DIFadd{Whereas }\DIFaddend much of the early work on image registration is aimed at the
problem of registering nearly identical images, say by minimizing a sum of
squared distances \DIFaddbegin \DIFadd{or maximizing normalized correlation}\DIFaddend , here we must confront several physical factors
simultaneously: misalignment, illumination variations, and \DIFdelbegin \DIFdel{certain amount of occlusion}\DIFdelend \DIFaddbegin \DIFadd{corrupted pixels}\DIFaddend .
...
is loosely motivated by theoretical results due to \DIFdelbegin \DIFdel{Candes }\DIFdelend \DIFaddbegin \DIFadd{Cand}\`{e}\DIFadd{s }\DIFaddend and Tao
...
image.  \DIFdelbegin \DIFdel{So, if one 's primary concern in designing the recognition system 
is achieving }\DIFdelend \DIFaddbegin \DIFadd{We argue that if one has the luxury of designing the acquisition system 
and the application demands }\DIFaddend a high recognition rate,
it \DIFdelbegin \DIFdel{makes no sense }\DIFdelend \DIFaddbegin \DIFadd{is then unwise }\DIFaddend to limit the gallery to a
...
projector based light stage.  \DIFdelbegin \DIFdel{Most light stages used for face recognition have been constructed for the purpose of creating public data sets to study illumination invariance }%DIFDELCMD < \cite{Georghiades2001-PAMI, Gross2008-FGR}%%%
\DIFdel{.  Many other light stages }\DIFdelend \DIFaddbegin \DIFadd{While similar systems }\DIFaddend have been used for
\DIFdelbegin \DIFdel{computer graphics purposes }%DIFDELCMD < \cite{debevec2000acquiring, jones2005performance}%%%
\DIFdel{. The light source can be moved around manually }%DIFDELCMD < \cite{masselus2002free}%%%
\DIFdel{, but this may result in poor consistency between users.  Structured light applications use projectors to directly illuminate the face (or other object) }%DIFDELCMD < \cite{zhang2002rapid} %%%
\DIFdel{for 3D reconstruction, but this is very disturbing to the user.  To }\DIFdelend \DIFaddbegin \DIFadd{other applications, to }\DIFaddend our knowledge, we
...
\DIFdelbegin \DIFdel{the two
}%DIFDELCMD < {\em %%%
\DIFdel{strongly coupled}%DIFDELCMD < } %%%
\DIFdel{issues of }\DIFdelend registration and illumination
can be \DIFdelbegin \DIFdel{naturally addressed within the }\DIFdelend \DIFaddbegin \DIFadd{simultaneously addressed within a robust }\DIFaddend sparse representation
...
that is capable of \DIFdelbegin \DIFdel{interpolating }\DIFdelend \DIFaddbegin \DIFadd{linearly representing }\DIFaddend typical indoor and outdoor
...
\DIFdelbegin \DIFdel{Finally, we }\DIFdelend \DIFaddbegin \DIFadd{We then }\DIFaddend demonstrate the effectiveness of the proposed new
...
controlled \DIFdelbegin \DIFdel{input }\DIFdelend \DIFaddbegin \DIFadd{probe }\DIFaddend images taken both indoors and outdoors, 
using \DIFdelbegin \DIFdel{labeled }\DIFdelend \DIFaddbegin \DIFadd{a gallery of 
}\DIFaddend frontal views of the subjects' faces under the proposed
illuminations\DIFdelbegin \DIFdel{for training and an }\DIFdelend \DIFaddbegin \DIFadd{. An }\DIFaddend off-the-shelf face
detector\footnote{We use the OpenCV
implementation of the Viola and Jones' face detector
\cite{Viola2004-IJCV}.} \DIFaddbegin \DIFadd{is used }\DIFaddend to detect faces in \DIFaddbegin \DIFadd{the test }\DIFaddend images.
...
\DIFdelend \DIFaddbegin \noindent{\em Organization of this paper:} \DIFaddend In Section \ref{sec:registration},
we derive our robust registration and recognition algorithm within the sparse
representation framework. We elaborate on \DIFdelbegin \DIFdel{the implementation issuesof the
algorithm}\DIFdelend \DIFaddbegin \DIFadd{algorithmic implementation issues}\DIFaddend ,
...
work\DIFdelbegin \DIFdel{in the
literature}\DIFdelend . Section \ref{sec:illumination} is dedicated to our training acquisition
system. Using this system, we investigate empirically \DIFdelbegin \DIFdel{the set of training
illuminations }\DIFdelend \DIFaddbegin \DIFadd{how many training
illuminations are }\DIFaddend required to handle practical illumination variations, and
...
database,\footnote{We assume the \DIFdelbegin \DIFdel{illuminations in the }\DIFdelend training \DIFdelbegin \DIFdel{set }\DIFdelend \DIFaddbegin \DIFadd{illuminations }\DIFaddend are sufficient. We will address how to ensure illumination
...
\centering
\DIFaddbeginFL {\small
\DIFaddendFL \begin{tabular}{cccc}
...
\DIFaddendFL \caption{{\bf Comparing alignment of a subject wearing sunglasses by
...
\DIFdelbeginFL %DIFDELCMD < \vspace{-.3in}
%DIFDELCMD < %%%
...
\DIFaddend \STATE {\bf Input:} \DIFdelbegin \DIFdel{Frontal training images $A_i \in \Re^{m\times n_i}, i=1,2,\ldots,K$ }\DIFdelend \DIFaddbegin \DIFadd{Training images $\{A_i \in \Re^{m\times n_i}\}_{i=1}^K$ }\DIFaddend for $K$ subjects,  a test image
...
$\ell^1$-minimization problem: \DIFdelbegin %DIFDELCMD < $$\hat{\x} %%%
\DIFdel{= \arg \min_{\x, \e} }%DIFDELCMD < \| \x \|%%%
\DIFdel{_1 + }%DIFDELCMD < \|\e\|%%%
\DIFdel{_1 }%DIFDELCMD < \;\; \subj \;\; \y %%%
\DIFdel{= A }%DIFDELCMD < \x %%%
\DIFdel{+ }%DIFDELCMD < \e%%%
\DIFdel{.}%DIFDELCMD < $$
\end{MATHMODE}%DIFAUXCMD
%DIFDELCMD < %%%
\begin{equation}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{\hspace{2mm}
$\hat{\x} = \arg \min_{\x, \e} \| \x \|_1 + \|\e\|_1 \;\; \subj \;\; \y = A \x + \e.$
}\DIFaddend \STATE Compute residuals $r_i(\y) = \| {\y} - {A}_i \, \delta_i(\hat{\x}) \|_2$ for $i = k_1, \dots, k_S$.
\STATE {\bf Output:} $\mbox{identity}(\y) = \arg\min_i r_i(\y)$.
\DIFaddbegin \end{small}
\DIFaddend \end{algorithmic}
\end{algorithm}

\DIFdelbegin \DIFdel{So far, these transformations have been expressed in a very
general form.  The transformation }\DIFdelend \DIFaddbegin \DIFadd{The transformation $\tau$ }\DIFaddend defines a mapping between the
coordinates of pixels in the large original image and a smaller
(un)warped image. The pixels of the small image \DIFdelbegin \DIFdel{get }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend stacked into a vector. \DIFaddbegin \DIFadd{To prevent aliasing
artifacts in the downsampled image, one should apply a
smoothing filter to the original image. }\DIFaddend For a simple implementation, a rectangular window with
...
\label{eq:alm}
\end{equation}
\DIFdelbegin \DIFdel{As one can see, although }\DIFdelend \DIFaddbegin \DIFadd{Although }\DIFaddend updating $\blamda$ is trivial,
...
method of multipliers} \DIFaddbegin \DIFadd{(ADM) }\DIFaddend \cite{Glowinski1975-TR}, which \DIFdelbegin \DIFdel{proposes to minimize }\DIFdelend \DIFaddbegin \DIFadd{alternates between minimizing }\DIFaddend $L_{\mu} (\x,\e,\blamda_k)$
\DIFdelbegin \DIFdel{with respect to }\DIFdelend \DIFaddbegin \DIFadd{over }\DIFaddend $\x$ \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(with }\DIFaddend $\e$ \DIFdelbegin \DIFdel{separately. Then the }\DIFdelend \DIFaddbegin \DIFadd{fixed) and over $\e$ (with $\x$ fixed). After solving these two subproblems, the Lagrange }\DIFaddend multiplier $\blamda$ is updated\DIFdelbegin \DIFdel{after just one iteration of minimization
with respect to the two primal variables, giving}\DIFdelend \DIFaddbegin \DIFadd{, yielding an iteration of the form}\DIFaddend :
\begin{equation}
\left \{
\begin{array}{lll}
...
\label{eq:adm}
\end{equation}
\DIFaddbegin \DIFadd{As the objective function is convex and alternation is between two
terms, this procedure is guaranteed to converge to a global optimum (see }\cite{YangJ2009-pp} \DIFadd{and references therein). 
}
...
\begin{algorithmic}[1]
\DIFaddbegin \begin{small}
\DIFaddend \STATE {\bf Input:} $\y \in \Re^m$, $A \in \Re^{m \times n}$,
...
\DIFdelbegin %DIFDELCMD < \item{\em 2D Deformation.}  %%%
...
\DIFdelbegin %DIFDELCMD < \item{\em 3D Pose Variation.} %%%
\DIFdelend \DIFaddbegin \noindent{3) \em 3D Pose Variation.} \DIFaddend As densely sampled pose and
...
\includegraphics[height=1in]{figures_cvpr/13} \DIFdelbeginFL %DIFDELCMD < \\ %%%
\DIFdelendFL \vspace{-.1in} \DIFaddbeginFL \\ 
\DIFaddendFL (a) & (b) & (c) & (d) & (e)\DIFdelbeginFL %DIFDELCMD < \vspace{3mm}%%%
\DIFdelendFL \\
...
\includegraphics[height=1in]{figures_cvpr/3} \DIFdelbeginFL %DIFDELCMD < \\ %%%
\DIFdelendFL \vspace{-.1in} \DIFaddbeginFL \\ 
...
}\label{fig:pose-alignment} \DIFdelbeginFL %DIFDELCMD < \vspace{-.3in}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{That paper assumes that convex, Lambertian objectsare photographed under distant illuminationswith a fixed pose.
...
and as a result,
the set of }\DIFdelend \DIFaddbegin \DIFadd{The main result of this paper is that for convex Lambertian objects, distant illuminations, and fixed pose,
%DIF > Under those assumptions, the incident and reflected light are represented using distributions on a sphere and can thus be studied in a spherical harmonic basis.
%DIF > The Lambertian nature of the object acts as a low-pass filter between the incident and reflected light distributions,
%DIF > and as a result, the set of images of the object end up lying very
%DIF > close to the subspace corresponding to low frequency spherical harmonics.\footnote{This is highly condensed; please refer to \cite{Basri2003-PAMI} for a  complete description of this result.}
all }\DIFaddend images of the object \DIFdelbegin \DIFdel{end up lying very
close to the subspace corresponding to low frequency spherical harmonics.}\footnote{\DIFdel{This is highly condensed; please refer to }%DIFDELCMD < \cite{Basri2003-PAMI} %%%
\DIFdel{for a  complete description of this result.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{Indeed, only }\DIFdelend \DIFaddbegin \DIFadd{can be well approximated by linear combinations of
}\DIFaddend nine (properly chosen) basis \DIFdelbegin \DIFdel{illuminations are sufficient to generate basis imagesthat span all possible images of the object to a good approximation}\DIFdelend \DIFaddbegin \DIFadd{images.  The basis images have mixed sign, and
their illuminations consist of the lowest frequency spherical harmonics}\DIFaddend .
...
% Non-physicality
\DIFdelbegin \DIFdel{Neither the basis illuminations, nor the basis images used
...
}\DIFdelend %DIF > Neither the basis illuminations, nor the basis images used
%DIF > in the Basri analysis are physical.  Like Fourier bases, they have negative components,
%DIF > and are thus not, at least directly, physically realizable.
% This alone is not
...
\DIFdelbegin \DIFdel{There are ways to engineer around this difficulty}\footnote{\DIFdel{such as allowing negative light while rendering from a 3D model, decomposing mixed-sign illuminations and images into positive negative parts, and enforcing positivity only in the output image}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{, but the photometric and geometric assumptions behind this model limit its applicability even more severely.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > There are ways to engineer around this difficulty\footnote{such as allowing negative light while rendering from a 3D model, decomposing mixed-sign illuminations and images into positive negative parts, and enforcing positivity only in the output image}, but the photometric and geometric assumptions behind this model limit its applicability even more severely.
...
%DIF < If the surface has a specular component, the relationship between incoming and outgoing light
}\DIFdelend %DIF > Specifically, self-shadowing and inter-reflection break the simple relationship between the outgoing light
%DIF > and the resulting images, and specularity even invalidates the representation of outgoing light that is used.
%DIF >  itself,
%DIF > since it is no longer independent of viewing angle.
%DIF > %If the surface has a specular component, the relationship between incoming and outgoing light
...
Fortunately, even with these effects, \DIFaddbegin \DIFadd{for most materials }\DIFaddend the relationship between
\DIFdelbegin \DIFdel{the space of illuminations and the space of images }\DIFdelend \DIFaddbegin \DIFadd{illumination and image }\DIFaddend is still linear\DIFdelbegin \DIFdel{; all we have to assume is that the image is dominated by scattered light.}%DIFDELCMD < \footnote{%%%
\DIFdel{Phenomena that break this assumption include fluorescence, the photochromic (``Transition'') lenses in some eyeglasses}\DIFdelend ,\DIFaddbegin \footnote{\DIFadd{Materials that break
...
incident light.}} \DIFadd{provided the sensor has a linear response curve.}\footnote{\DIFadd{Proper handling of gamma encoding is an important consideration for
practitioners.  Most cameras apply a non-linear }\DIFaddend and \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{often undocumented response
curve to captured images.  A slight degradation of performance will occur if
gamma compressed images are treated as if they were linear.  We recommend }\DIFaddend the \DIFdelbegin \DIFdel{infrared light radiated by all warm objects}\DIFdelend \DIFaddbegin \DIFadd{use
of cameras with well documented response curves that can be inverted when the 
image file is loaded}\DIFaddend .\DIFdelbegin %DIFDELCMD < }%%%
\DIFdel{.
Note that while }\DIFdelend \DIFaddbegin }
\DIFadd{For a more in-depth study 
of the relationship between illumination and images, we refer the reader to
}\cite{belhumeur1998set}\DIFadd{.
While }\DIFaddend the relationship between illuminations and images is linear,
...
\subsection{\DIFdelbegin \DIFdel{Capturing a Sufficient Set of Training
Illuminations}\DIFdelend \DIFaddbegin \DIFadd{Projector-based Illumination System}\DIFaddend } 
\DIFdelbegin \DIFdel{To this end, we }\DIFdelend \DIFaddbegin 

\DIFadd{We }\DIFaddend have designed a system that can acquire frontal images of a subject while
...
\end{itemize}
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \centerline{\hspace{-0.1in}\includegraphics[height=2.3in]{figures_cvpr/camera_rig.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\hspace{-0.1in}\includegraphics[height=2.3in]{figures_cvpr/camera_rig.pdf}\vspace{-.2in}}
\DIFaddendFL \caption{{\bf Training acquisition system:} Four projectors and two cameras controlled by one computer.}
\DIFdelbeginFL %DIFDELCMD < \vspace{-.5in}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \vspace{-.3in}
\DIFaddendFL \label{fig:system}
\end{figure}
...
\DIFaddbegin {\em \DIFadd{Relationship to existing work:}} \DIFadd{Most light stages used for face recognition have
been constructed for the purpose of creating public data sets to study
illumination invariance }\cite{Georghiades2001-PAMI, Gross2008-FGR}\DIFadd{.  Many other
light stages have been used for computer graphics purposes
}\cite{debevec2000acquiring, jones2005performance}\DIFadd{.
The light source can be
moved around manually }\cite{masselus2002free}\DIFadd{, but this may result in poor
consistency of illuminations between users.  Structured light applications use projectors to
directly illuminate the face (or other object) }\cite{zhang2002rapid} \DIFadd{for 3D
reconstruction, but this is very disturbing to the user. 
Y.}\ \DIFadd{Schechner }\cite{schechner2007multiplexing}
\DIFadd{studies techniques for multiplexing illumination that can dramatically reduce the noise 
...
\end{tabular}\DIFdelbeginFL %DIFDELCMD < \vspace{2mm}
%DIFDELCMD < %%%
...
\label{fig:illumination-patterns}%DIF < \vspace{-.5in}
...
\DIFaddendFL \end{figure}
 ...
 \DIFdelbegin \subsection{\DIFdel{Chosen Illumination Patterns}} %DIFAUXCMD
...
\DIFdelbegin \subsection{\DIFdel{Compensating for Gamma Compression}} %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend \section{Tests on Public Databases}\label{sec:multipie}
...
treated as ``\DIFdelbegin \DIFdel{imposters}\DIFdelend \DIFaddbegin \DIFadd{impostors}\DIFaddend '', or invalid images. For each of the
...
distinct times }\DIFdelend over a period of several months. The dataset is
...
achieves recognition rates above $90\%$ for all three sessions\DIFdelbegin \DIFdel{,
with input }%DIFDELCMD < {\em %%%
\DIFdel{directly}%DIFDELCMD < } %%%
\DIFdel{obtained from }\DIFdelend \DIFaddbegin \DIFadd{.
For the test images, 
alignment was initialized automatically via }\DIFaddend the Viola and Jones'
face detector\DIFdelbegin \DIFdel{; there was no manual intervention for the test images. }\DIFdelend \DIFaddbegin \DIFadd{. 
...
%DIFDELCMD < \hline
%DIFDELCMD < Recognition rate & Session 2 & Session 3 & Session 4  \\
%DIFDELCMD < \hline
%DIFDELCMD < \hline
%DIFDELCMD < LDA$_d$ (LDA$_m$) & 5.1 (49.4)\%  & 5.9 (44.3)\% & 4.3 (47.9)\%  \\
%DIFDELCMD < \hline
%DIFDELCMD < NN$_d$ (NN$_m$)  & 26.4 (67.3)\% & 24.7 (66.2)\% & 21.9 (62.8)\%  \\
%DIFDELCMD < \hline
%DIFDELCMD < NS$_d$ (NS$_m$) &  30.8 (77.6)\% & 29.4 (74.3)\% & 24.6 (73.4)\% \\
%DIFDELCMD < \hline
%DIFDELCMD < \cite{Yang2010-CVPR} & {\bf 95.2}\% & 93.4\% & 95.1\% \\
%DIFDELCMD < \hline
%DIFDELCMD < {Algorithm 1} & 93.9\% & 93.8\% & 92.3\% \\
%DIFDELCMD < \hline
%DIFDELCMD < {Algorithm 1 with improved window} & 95.0\% & {\bf 96.3}\% & {\bf 97.3}\% \\
%DIFDELCMD < \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:MultiPIE-recognition} }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \vspace{-.2in}
\centerline{
...
\centering
\DIFaddbeginFL {\small
\DIFaddendFL \begin{tabular}{cccccc}
...
\includegraphics[width=0.9in,clip=true]{figures_pami/multipie_failed/175_02_01_051_08.png} \DIFaddbeginFL \vspace{-.1in} \DIFaddendFL \\
(a) & (b) & (c) & (d) & (e) & (f) \DIFaddbeginFL \vspace{-.1in}
\DIFaddendFL \end{tabular}
\DIFaddbeginFL }
\DIFaddendFL \caption{{\bf Representative \DIFdelbeginFL \DIFdelFL{examples of failed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{failures from }\DIFaddendFL Multi-PIE\DIFdelbeginFL \DIFdelFL{subjects}\DIFdelendFL }. {\bf Top:} training from Session 1; {\bf Bottom:} test images from Session 2. Due to \DIFdelbeginFL \DIFdelFL{change of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{changes in }\DIFaddendFL hair\DIFdelbeginFL \DIFdelFL{style}\DIFdelendFL , glasses, beard, or pose, our alignment fails on these subjects regardless of test image illumination.}
\label{fig:failed-examples}
%DIF < \vspace{-.8in}
\DIFaddbeginFL \vspace{-.3in}
\DIFaddendFL \end{figure}
\DIFdelbegin \subsection{\DIFdel{Improving the Performance by Choosing Different Sampling Windows}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend 

\DIFaddbegin \subsection{\DIFadd{Improving the Performance by Choosing Different Sampling Windows}}
\DIFaddend Our algorithm's errors are mostly caused by a few subjects who
...
\centering
\DIFaddbeginFL {\small
\DIFaddendFL \begin{tabular}{cc}
\includegraphics[height=1.5in]{figures_pami/example.png} &
\includegraphics[height=1.5in]{figures_pami/example_new.png} \DIFaddbeginFL \vspace{-.1in} \DIFaddendFL \\
(a) & (b) \DIFaddbeginFL \vspace{-.1in}
\DIFaddendFL \end{tabular}
\DIFaddbeginFL }
\DIFaddendFL \caption{{\bf Choosing different sampling windows.} (a) The default window. (b) The newly proposed window.}
\label{fig:new-mask}
%DIF < \vspace{-.6in}
\DIFaddbeginFL \vspace{-.3in}
\DIFaddendFL \end{figure}
...
with the new window. \DIFdelbegin %DIFDELCMD < 
...
also observed for SRC in }%DIFDELCMD < \cite{Wright2009-PAMI}%%%
...
%DIFDELCMD < \includegraphics[width=3in]{figures_pami/pami_roc2.png}
%DIFDELCMD < }
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < {\bf %%%
\DIFdel{ROC curves}%DIFDELCMD < } %%%
\DIFdel{for subject validation on Multi-PIE database.}}%DIFAUXCMD
%DIFDELCMD < \label{fig:roc-multipie}
%DIFDELCMD < %%%
%DIF < \vspace{-.6in}
...
We also compare our result to the \DIFdelbegin \DIFdel{most }\DIFdelend \DIFaddbegin \DIFadd{very }\DIFaddend recent work
...
\DIFaddbegin \DIFadd{Finally, we compare our result to Local Binary Patterns (LBP)
}\cite{Ahonen2006-PAMI}\DIFadd{, a local appearance descriptor which is
...
}\cite{Tan06facerecognition}\DIFadd{. In this paper, we follow the same
steps as in }\cite{Ahonen2006-PAMI} \DIFadd{to construct an LBP
...
}\cite{Ahonen2006-PAMI}\DIFadd{, the recognition is performed using a
...
\DIFadd{Although our algorithm is not designed for
...
}\cite{phillips1998feret}\DIFadd{, which contains five standard
...
using manually marked eye coordinates }\cite{Deng2010-PR}\DIFadd{. In
...
\begin{table}
\caption{\DIFaddFL{Recognition rates on single gallery image FERET
...
\DIFadd{As expected, our algorithm does not perform well except for
...
directly obtained from the face detector }\cite{Wolf2008-ECCV}\DIFadd{. 
}

\DIFadd{All of these experimental results confirm that
...
\subsection{\DIFadd{Subject Validation}}

\DIFadd{We test the algorithms' ability to reject invalid images of the
88 subjects not appearing in the training database. As
mentioned before, the \emph{sparsity concentration index} (SCI)
is used as the outlier rejection rule. Given the sparse
representation $\x$ of a test image with respect to $K$
training classes, the SCI measures how concentrated the
coefficients are on a single class in the dataset and is
defined as in }\cite{Wright2009-PAMI}\DIFadd{:
}\begin{displaymath}
\DIFadd{\textup{SCI}(}\x\DIFadd{) \doteq \frac{K \cdot \max_i \|\delta_i(\x)\|_1 /
\|\x\|_1 - 1}{K - 1} \in }[\DIFadd{0,1}] \DIFadd{.
}\end{displaymath}
\DIFadd{It is easy to see that if $\textup{SCI}(\x) = 1$, the test
image is represented using images from one single subject
class; if $\textup{SCI}(\x) = 0$, the coefficients are spread
evenly over all classes. Thus, we can choose a threshold $t \in
[0,1]$ for the proposed method and accept a test image as valid
...
\DIFadd{Figure \ref{fig:roc-multipie} plots the receiver operating
characteristic (ROC) curves, which are generated by sweeping
the threshold $t$ through the entire range of possible values
for each algorithm.}\footnote{\DIFadd{Rejecting invalid images not in
the entire database is much more difficult than deciding if two
face images are the same subject. Figure 8 should not be
confused with typical ROC curves for face similarity, e.g.,
}\cite{PhillipsP2007}\DIFadd{.}} \DIFadd{We can see that SCI based approach
significantly outperforms the other three algorithms. Similar
contrasts between our algorithm and baseline algorithms were
also observed for SRC in }\cite{Wright2009-PAMI}\DIFadd{, though on much
smaller datasets.
}
...
\caption{{\bf \DIFaddFL{ROC curves}} \DIFaddFL{for subject validation on Multi-PIE database.}}\label{fig:roc-multipie}
\vspace{-.6in}
\end{figure}

\DIFaddend \subsection{Recognition with Synthetic Random Block Occlusion}
...
recognition rate.\DIFaddbegin \vspace{-.1in}
\DIFaddend 
...
%DIF < \vspace{-.25in}
 \DIFaddbeginFL \vspace{-.2in}
 \DIFaddendFL \caption{{\bf Representative examples of categories C1-C3}. One row for each category.}\label{fig:examples1-3}
 \DIFaddbeginFL \vspace{-.1in}
\DIFaddendFL \end{figure}
...
\DIFdelbeginFL %DIFDELCMD < \vspace{-.35in}
%DIFDELCMD < %%%
...
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{Test Category }%DIFDELCMD < & %%%
\DIFdel{C1 }%DIFDELCMD < & %%%
\DIFdel{C2 }%DIFDELCMD < & %%%
\DIFdel{C3 }%DIFDELCMD < & %%%
\DIFdel{C4  }%DIFDELCMD < \\
...
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{Recognition rate }%DIFDELCMD < & %%%
\DIFdel{98.4}%DIFDELCMD < \% & %%%
\DIFdel{95.8}%DIFDELCMD < \% & %%%
\DIFdel{95.1}%DIFDELCMD < \% & %%%
\DIFdel{40.9}%DIFDELCMD < \% \\
%DIFDELCMD < \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:UIUC-recognition}
%DIFDELCMD < \vspace{-.25in}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend Since our focus is on face
...
\vspace{-.1in}	
\centering \caption{\DIFaddFL{Recognition rates on our own database.}}
\vspace{-.2in}
\begin{tabular}{|c|c|c|c|c| }
\hline
\DIFaddFL{Test Category }& \DIFaddFL{C1 }& \DIFaddFL{C2 }& \DIFaddFL{C3 }& \DIFaddFL{C4  }\\
\hline
\hline
\DIFaddFL{Recognition Rate }& \DIFaddFL{98.4}\% & \DIFaddFL{95.8}\% & \DIFaddFL{95.1}\% & \DIFaddFL{40.9}\% \\
\hline
\end{tabular}
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{Using a well-though-out combination of existing ideas
(iterative image alignment, l1-error function, SRC, using projectors for
illumination), we }\DIFaddend have proposed a \DIFdelbegin \DIFdel{new algorithm and }\DIFdelend system for recognizing human faces
from images taken under practical conditions \DIFdelbegin \DIFdel{. The
proposed system is very }%DIFDELCMD < {\em %%%
\DIFdel{simple}%DIFDELCMD < }%%%
\DIFdel{, }%DIFDELCMD < {\em %%%
\DIFdel{scalable}%DIFDELCMD < } %%%
\DIFdel{both in
terms of computational complexity and recognition performance.
The system is compatible with off-the-shelf face
detectors. 
}\DIFdelend \DIFaddbegin \DIFadd{that is conceptually simple, well
motivated, and competitive with state-of-the-art recognition systems for access
control scenarios. 
}

\DIFaddend The system achieves extremely \DIFdelbegin %DIFDELCMD < {\em %%%
\DIFdel{stable }%DIFDELCMD < } %%%
...
07-01676, and ONR N00014-09-1-0230. \DIFdelbegin \DIFdel{John Wright would like to
thank }\DIFdelend \DIFaddbegin \DIFadd{JW thanks }\DIFaddend Allen Yang of UC Berkeley EECS and Robert Fossum of UIUC
Mathematics for discussions related to this work\DIFdelbegin \DIFdel{. He would like
to acknowledge }\DIFdelend \DIFaddbegin \DIFadd{, and 
acknowledges }\DIFaddend support from a Microsoft \DIFdelbegin \DIFdel{Research }\DIFdelend Fellowship and
the Lemelson-Illinois Student Prize. %DIF > } %\vspace{-0.2in}
