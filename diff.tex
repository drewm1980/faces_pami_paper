...
occlusion or corruption by solving a simple convex program.  \DIFaddbegin \DIFadd{For this
reason, the final recognition stage of our proposed system will consist of SRC,
and the alignment stage that precedes it will be based on similar techniques.
}\DIFaddend 
...
\DIFaddend Unfortunately, while SRC achieves impressive results, it does not deal with misalignment
...
a sufficiently accurate 3D model, for access control applications where the
user is cooperative in aligning their head to the camera, robustness to
moderate misalignment is sufficient.  Furthermore, many 3D techniques rely on
multiple views to estimate their model, and may have trouble if all of the
training images are captured from one pose.  Note that images of people's faces
under varying illumination contain shape-related discriminative information,
and this information can be leveraged by 2D algorithms even if shape is not
...
image.  \DIFdelbegin \DIFdel{So, if one 's primary concern in designing the recognition system is achieving }\DIFdelend \DIFaddbegin \DIFadd{We argue that if one has the luxury of designinig the recognition algorithm,
but also the acquisition system, and the application demands }\DIFaddend a high recognition rate,
it \DIFdelbegin \DIFdel{makes no sense }\DIFdelend \DIFaddbegin \DIFadd{is inadvisable }\DIFaddend to limit the gallery to a
...
\DIFdelend \DIFaddbegin \DIFadd{mutually confounding }\DIFaddend issues of registration and illumination
...
}{\bf \DIFadd{DISCUSSION OF ZIHAN'S LBP IMPLEMENTATION GOES HERE}}
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Our algorithm is not designed for recognition when there is only a single
gallery image per user, and will not achieve competitive performance,
especially if the illumination in the test image is significantly different
from the gallery image. For applications that are constrained to a single
gallery image per user, algorithms such as LBP are much more appropriate.  We
demonstrate this by running Algorithm 1, as well as our LBP implementation, on
three standard subsets of the FERET }\cite{phillips1998feret} \DIFadd{dataset,
summarized in Table \ref{tab:FERET-recognition}.  
}\begin{table}
\caption{\DIFaddFL{Recognition rates on single gallery image FERET dataset}}
\centerline{
\begin{tabular}{|c|c|c|c|c| }
\hline
Recognition rate & fb & fc & ? \\
\hline
\hline
{LBP} & ?\% & ?\% & ?\% \\
\hline
