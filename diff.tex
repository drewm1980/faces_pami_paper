...
\PARstart{T}{here} \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{has been }\DIFaddend a historical tendency for face recognition algorithms to work well under laboratory conditions  \DIFdelbegin \DIFdel{but degrade }\DIFdelend \DIFaddbegin \DIFadd{(well aligned images, similar lighting conditions, and little change in appearances) but degrade drastically }\DIFaddend when tested in less-controlled \DIFdelbegin \DIFdel{environments.  }\DIFdelend \DIFaddbegin \DIFadd{real-world environments. Although achieving high recognition performance under uncontrolled conditions and with limited gallery images has been the traditional goal for face recognition, existing techniques have not yet been able to meet the performance requirements of high-stake security-related applications. }\DIFaddend While there have been \DIFdelbegin \DIFdel{several }\DIFdelend \DIFaddbegin \DIFadd{many successes on less-demanding applications such as online image search and family photo album organization (e.g. the freely available Google's Picassa, Microsoft's Photo Gallery, and Apple's iPhoto), there have been many }\DIFaddend high profile failed trials of face recognition technology \DIFdelbegin \DIFdel{for extremely difficult }\DIFdelend \DIFaddbegin \DIFadd{worldwide for extremely challenging }\DIFaddend mass surveillance/watch-list applications\DIFaddbegin \DIFadd{. 
}

\DIFadd{Somewhat surprisingly}\DIFaddend , face recognition has not \DIFdelbegin \DIFdel{even }\DIFdelend seen widespread applications \DIFaddbegin \DIFadd{or success }\DIFaddend in more tractable \DIFaddbegin \DIFadd{security }\DIFaddend settings, such as access control for \DIFdelbegin \DIFdel{buildings}\DIFdelend \DIFaddbegin \DIFadd{secure facilities (e.g., prisons, office buildings)}\DIFaddend , computer systems, automobiles, or even automatic teller machines\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{,  where controlled gallery images can be obtained in advance.}\DIFaddend \footnote{Face recognition scenarios where both gallery and test data are uncontrolled \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{continue to be }\DIFaddend an active research area \DIFdelbegin \DIFdel{as well }\DIFdelend \cite{LFW}\DIFaddbegin \DIFadd{, and becomes more popular recently due to aforementioned online applications}\DIFaddend .}  These applications are very interesting due to their potential sociological \DIFaddbegin \DIFadd{and technological }\DIFaddend impact.  They are also qualitatively different from \DIFdelbegin \DIFdel{surveillance }\DIFdelend \DIFaddbegin \DIFadd{mass surveillance or online image }\DIFaddend applications because they demand extremely reliable systems \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{whose performance must compete with other biometric measures such as fingerprint and iris. Now }\DIFaddend because the gallery subjects are allies, rather than opponents, of the recognition system\DIFdelbegin \DIFdel{.  This }\DIFdelend \DIFaddbegin \DIFadd{, this }\DIFaddend creates the possibility of carefully controlling the acquisition of the training data \DIFdelbegin \DIFdel{, even if the testing data may be less controlled for usability reasons}\DIFdelend \DIFaddbegin \DIFadd{(as we normally do for all other biometrics). However, unlike other biometrics, face recognition has the potential in working with much less controlled testing data so that the access control system can be made more discreet and much less intrusive than using other biometrics. To some extent, the goal of this paper is to show how a reliable face recognition system can be built for this admittedly restrictive but important scenario}\DIFaddend . 

Classical holistic subspace based \DIFaddbegin \DIFadd{face recognition }\DIFaddend methods \cite{Turk1991-CVPR,Belhumeur1997-PAMI} are well known for their speed and simplicity, as well as for their natural extension to linear illumination
...
squared distances \DIFaddbegin \DIFadd{or maximize normalized correlation}\DIFaddend , here we must confront several physical factors
...
method of multipliers} \DIFaddbegin \DIFadd{(ADM) }\DIFaddend \cite{Glowinski1975-TR}, which proposes to minimize $L_{\mu} (\x,\e,\blamda_k)$
with respect to $\x$ and $\e$ separately. \DIFaddbegin \DIFadd{As the objective function is convex and alternation is between two
terms, this procedure has been proven to converge to the global optima (see }\cite{YangJ2009-pp} \DIFadd{and references therein). 
}
...
\DIFdelbegin \DIFdel{; all we have to assume is that the image is dominated by scattered light. 
}\DIFdelend \DIFaddbegin \footnote{\DIFadd{Materials that break
...
incident light.}}\DIFadd{, 
and that the sensor has a linear response curve. 
}\DIFaddend \footnote{\DIFdelbegin \DIFdel{Phenomena that break this assumption include fluorescence, the photochromic (``Transition'') lenses in some eyeglasses, }\DIFdelend \DIFaddbegin \DIFadd{ Proper handling of gamma encoding is an important consideration for
...
\DIFadd{For a more indepth study 
...
\DIFadd{Most light stages used for face recognition have
...
\end{itemize}
\begin{figure}
\centerline{\hspace{-0.1in}\includegraphics[height=2.3in]{figures_cvpr/camera_rig.pdf}}
...
steps as in }\cite{Ahonen2006-PAMI} \DIFadd{to construct a LBP
...
\DIFadd{As expected, our algorithm doesn't perform well except for
`fb', in which case the illumination is similar to the training and 
