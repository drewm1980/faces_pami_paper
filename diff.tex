...
\PARstart{T}{here} \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{has been }\DIFaddend a historical tendency for face recognition algorithms to work well under laboratory conditions  \DIFdelbegin \DIFdel{but degrade }\DIFdelend \DIFaddbegin \DIFadd{(well aligned images, similar lighting conditions, and little change in appearance) but degrade drastically }\DIFaddend when tested in less-controlled \DIFdelbegin \DIFdel{environments.  }\DIFdelend \DIFaddbegin \DIFadd{real-world environments. Although achieving high recognition performance under uncontrolled conditions and with limited gallery images has been the traditional goal of face recognition research, existing techniques are not yet able to meet the performance requirements of high-stakes security-related applications. }\DIFaddend While there have been \DIFdelbegin \DIFdel{several high profile }\DIFdelend \DIFaddbegin \DIFadd{many successes in less-demanding applications such as online image search and family photo album organization (e.g.}\ \DIFadd{Google Picassa, Microsoft Photo Gallery, and Apple iPhoto), there have been a number of high-profile }\DIFaddend failed trials of face recognition technology \DIFdelbegin \DIFdel{for extremely difficult }\DIFdelend \DIFaddbegin \DIFadd{in extremely challenging }\DIFaddend mass surveillance/watch-list applications\DIFaddbegin \DIFadd{. 
}

\DIFadd{Somewhat surprisingly}\DIFaddend , face recognition has not \DIFdelbegin \DIFdel{even }\DIFdelend seen widespread applications \DIFaddbegin \DIFadd{or success }\DIFaddend in more tractable \DIFaddbegin \DIFadd{security }\DIFaddend settings, such as access control for \DIFdelbegin \DIFdel{buildings}\DIFdelend \DIFaddbegin \DIFadd{secure facilities (e.g., prisons, office buildings)}\DIFaddend , computer systems, automobiles, or \DIFdelbegin \DIFdel{even }\DIFdelend automatic teller machines\DIFdelbegin \DIFdel{.}\footnote{\DIFdel{Face recognition scenarios where both gallery and test data are uncontrolled is an active research area as well }%DIFDELCMD < \cite{LFW}%%%
\DIFdel{.}}  %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{, where controlled gallery images can be obtained in advance.
%DIF > \footnote{Face recognition scenarios where both gallery and test data are uncontrolled continue to be an active research area \cite{LFW}, and has become more popular recently due to the aforementioned online applications.}  
}\DIFaddend These applications are very interesting due to their potential sociological \DIFaddbegin \DIFadd{and technological }\DIFaddend impact.  They are also qualitatively different from \DIFdelbegin \DIFdel{surveillance }\DIFdelend \DIFaddbegin \DIFadd{mass surveillance or online image }\DIFaddend applications because they demand extremely reliable systems \DIFdelbegin \DIFdel{, and because }\DIFdelend \DIFaddbegin \DIFadd{whose performance must compete with other biometric measures such as fingerprint and iris. Because }\DIFaddend the gallery subjects are allies, rather than opponents, of the recognition system\DIFdelbegin \DIFdel{.  This }\DIFdelend \DIFaddbegin \DIFadd{, this }\DIFaddend creates the possibility of carefully controlling the acquisition of the training data \DIFdelbegin \DIFdel{, even if the testing data may be less controlledfor usability reasons}\DIFdelend \DIFaddbegin \DIFadd{(as is common practice for other biometrics). However, unlike other biometrics, face recognition has the potential of working with test data that is much less controlled, allowing the access control system to be made more discreet and much less intrusive. To some extent, the goal of this paper is to show how a reliable face recognition system can be built for this admittedly restrictive, but still important, scenario}\DIFaddend . 

Classical holistic \DIFdelbegin \DIFdel{subspace based }\DIFdelend \DIFaddbegin \DIFadd{subspace-based face recognition }\DIFaddend methods \cite{Turk1991-CVPR,Belhumeur1997-PAMI} are well known for their speed and simplicity, as well as for their natural extension to linear illumination
models.  However, their performance has been shown to be extremely brittle not
only to alignment variation, but to \DIFdelbegin \DIFdel{occlusions}\DIFdelend \DIFaddbegin \DIFadd{even minor occlusions caused by, say, a
wisp of hair, a blinked eye, or mouth that is slightly open}\DIFaddend . One promising
...
%DIF > For this reason, the final recognition stage of our proposed system will consist of SRC,
%DIF > and the alignment stage that precedes it will be based on similar techniques.
...
\ref{fig:promo}(top). In addition, in order to \DIFdelbegin \DIFdel{linearly }\DIFdelend span
...
than sufficient}\DIFaddend .   Note that \DIFdelbegin \DIFdel{images of people's }\DIFdelend \DIFaddbegin \DIFadd{according to the principle of photometric stereo, images of }\DIFaddend faces
...
}\DIFaddend even if shape is not reconstructed explicitly. \DIFaddbegin \DIFadd{In addition, the proposed sparsity-based robust
...
\DIFdelbegin \DIFdel{parameters}\DIFdelend \DIFaddbegin \DIFadd{degrees of freedom (say an affine transform)}\DIFaddend , and to use the training images themselves 
...
\DIFdelend \DIFaddbegin \DIFadd{mutually compounding }\DIFaddend issues of registration and illumination
can be naturally addressed within \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a robust }\DIFaddend sparse representation
...
\DIFdelend \DIFaddbegin \noindent{\em Organization of this paper:} \DIFaddend In Section
...
The Lambertian nature of the object acts as a low-pass filter between the incident and reflected light distributions, and as a result,
...
\DIFaddbegin \DIFadd{To demonstrate that the SCI-based recognition step is indeed beneficial
...
\hline
{Algorithm 1 w/o SCI} & 90.7\% & 89.6\% & 87.5\% \\
\hline
...
\DIFadd{Meanwhile, although our algorithm is not designed for
...
\DIFadd{All these experimental results re-emphasize the importance that
both illumination and alignment need to be simultaneously handled
well in order to obtain accurate face recognition, even when there is
...
\DIFdelbegin \DIFdel{We have proposed a new algorithm and }\DIFdelend \DIFaddbegin \DIFadd{Using a well-though-out combination of existing ideas
(iterative image alignment, l1-error function, SRC, using projectors for
illumination), one can build a }\DIFaddend system for recognizing human faces
...
\DIFaddend The system achieves extremely {\em stable} performance under
